...
// ==========================================
// 1. DATA SCHEMA INTERFACES & CONTEXT
// ==========================================
...
export const NGOProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Real-time developer inspection terminal logging
  ...
  const loadDataFromBackend = async () => {
    ...
    try {
      ...
      // Fallback Seed Data matching requirements directly
      setVolunteers([
...
// ==========================================
// 3. CORE INTERACTIVE DASHBOARD COMPONENT
// ==========================================
...
function NGOManagementDashboard() {
  ...
  const exportVolunteersCSV = () => {
    let content = "data:text/csv;charset=utf-8,ID,Full Name,Email,Mobile,City,Skills,Status\n";
    volunteers.forEach(v => {
      content += `${v.id},"${v.name}","${v.email}",${v.mobile},"${v.city}","${v.skills}",${v.status}\n`;
    });
    const downloadLink = document.createElement("a");
    downloadLink.setAttribute("href", encodeURI(content));
    downloadLink.setAttribute("download", `NGO_Volunteer_Roster.csv`);
    document.body.appendChild(downloadLink);
    downloadLink.click();
    document.body.removeChild(downloadLink);
    triggerToast("Volunteer database compiled and downloaded!");
  };

  const exportParticipationCSV = () => {
    let content = "data:text/csv;charset=utf-8,Event ID,Event Name,Location,Date,Roster Capacity,Enrolled Count,Status\n";
    events.forEach(ev => {
      const joins = enrollments.filter(en => en.eventId === ev.id).length;
      content += `${ev.id},"${ev.name}","${ev.location}",${ev.date},${ev.requiredVolunteers},${joins},${ev.status}\n`;
    });
    const downloadLink = document.createElement("a");
    downloadLink.setAttribute("href", encodeURI(content));
    downloadLink.setAttribute("download", `NGO_Event_Enrollments.csv`);
    document.body.appendChild(downloadLink);
    downloadLink.click();
    document.body.removeChild(downloadLink);
    triggerToast("Event analysis reports compiled and downloaded!");
  };
  ...
  const handleAddVolunteer = async (e: React.FormEvent) => {
    ...
    if (volunteers.some(v => v.email.toLowerCase() === volInput.email.toLowerCase())) {
      triggerToast("Error: Email address already registered.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      {toastMessage && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-5 py-3 rounded-xl shadow-xl text-xs font-semibold flex items-center gap-2 animate-bounce">
          <span className="w-2 h-2 rounded-full bg-[#2a9d8f]"></span>
          {toastMessage}
        </div>
      )}

      {/* Responsive Brand Header Area */}
      ...
      <main className="max-w-7xl mx-auto py-8 px-6 grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white border border-[#2a9d8f] rounded-2xl p-6 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 pb-3 gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm font-sans">Welcome back, {activeVolunteerUser?.name}!</h3>
          ...
          {/* Interactive Statistics Dashboard Reports */}
          <section className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-2 border-b border-slate-100">
              ...
              <div className="flex gap-2">
                <button onClick={exportVolunteersCSV} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider border border-slate-200">
                  📁 Volunteers CSV
                </button>
                <button onClick={exportParticipationCSV} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider border border-slate-200">
                  📊 Analytics CSV
                </button>
              </div>
            </div>
          ...
          <section className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-2 border-b border-slate-200">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                <select 
                  ...
                  className="px-2 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 outline-none hover:bg-slate-50 cursor-pointer"
                >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {filteredEvents.map(ev => {
                ...
                return (
                  <div key={ev.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                    <div>
                      <div className="flex justify-between items-center gap-2 mb-2">
                        ...
                        {roleMode === 'admin' && (
                          <div className="flex gap-2 text-xs">
                            <button onClick={() => setEditingEvent(ev)} className="text-slate-400 hover:text-slate-600" title="Modify parameters">📝</button>
                            <button onClick={() => { deleteEvent(ev.id); triggerToast("Event deleted successfully"); }} className="text-slate-400 hover:text-red-600" title="Drop drive">🗑️</button>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 space-y-3">
                      {assignedVolunteers.length > 0 && (
                        <div className="space-y-1">
                          <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest block">Allocated Personnel</span>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <button 
                          ...
                          className="w-full bg-[#2a9d8f] hover:bg-[#21867a] disabled:bg-slate-100 disabled:text-slate-400 text-white font-bold py-1.5 rounded-lg text-[9px] uppercase tracking-wider"
                        >
                          Enroll Slot
                        </button>
                        <select 
          ...
          {/* Onboarded Staff & Volunteers Directory */}
          <section className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-2 gap-3">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Onboarded Volunteer Team</h3>
                <p className="text-xs text-slate-400">Review user competencies and active assignments mapping parameters.</p>
              </div>
              <input 
                type="text" 
                value={searchVol}
                onChange={e => setSearchVol(e.target.value)}
                placeholder="Search by skill, city or name..." 
                className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-[#2a9d8f] w-full sm:w-64"
              />
            </div>

            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
              {filteredVolunteers.map(vol => {
                const totalJoined = enrollments.filter(en => en.volunteerId === vol.id).length;
                return (
                  <div key={vol.id} className="bg-[#fef5f5] hover:bg-white border border-red-50 hover:border-slate-200 p-5 rounded-2xl shadow-sm transition-all flex flex-col justify-between group">
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <div className="w-10 h-10 rounded-full bg-[#2a9d8f]/10 text-[#2a9d8f] flex items-center justify-center font-bold text-sm">
                          {vol.name.charAt(0)}
                        </div>
                        <div>
                          <h4 className="font-bold text-slate-800 group-hover:text-[#e63946] transition-colors">{vol.name}</h4>
                          <p className="text-[9px] font-black text-[#2a9d8f] uppercase tracking-wider">{vol.city} Hub</p>
                        </div>
                      </div>

                      <div className="space-y-1.5 text-xs text-slate-600 border-t border-dashed border-slate-200/50 pt-3">
                        <p className="truncate">📧 {vol.email}</p>
                        <p>📞 {vol.mobile}</p>
                        <p className="font-bold text-slate-500 text-[10px]">🗓️ Assigned: {totalJoined} targets</p>
                        <div className="mt-2 pt-2 border-t border-slate-100">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Expertise Tags</span>
                          <p className="font-bold text-[#e63946] text-[11px]">{vol.skills}</p>
                        </div>
                      </div>
                    </div>

                    <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-[10px]">
                      <span className="inline-block px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[9px] font-extrabold uppercase rounded border border-emerald-100">{vol.status}</span>
                      <span className="text-slate-400 font-bold">UID: {vol.id.toString().padStart(4, '0')}</span>
                    </div>
                  </div>
                );
              })}
        ...
        {/* Right Column Forms Segment */}
        <div className="lg:col-span-4 space-y-6">
          
          {/* SQL / REST API Live Log Terminal */}
          <div className="bg-slate-900 text-slate-300 rounded-2xl p-5 shadow-lg space-y-3 font-mono text-[9px]">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-white font-bold uppercase">Telemetry Stream Monitor</span>
          ...
          {/* Administration: Create Event Drive */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <form onSubmit={handleAddEvent} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full mt-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-[#e63946] disabled:cursor-not-allowed font-semibold text-slate-600 hover:bg-slate-100 cursor-pointer"
                  >
                    <option value="Upcoming">Upcoming</option>
                    <option value="Ongoing">Ongoing</option>
                    <option value="Completed">Completed</option>
              ...
              <button 
                type="submit" 
                disabled={roleMode !== 'admin'}
                className="w-full bg-[#e63946] hover:bg-[#c92c38] text-white disabled:bg-slate-100 disabled:text-slate-400 font-bold py-2 rounded-xl text-xs uppercase tracking-wider transition-all disabled:cursor-not-allowed shadow-sm"
              >
                Publish Event
              </button>
            </form>
          ...
          {/* Volunteer Registration Onboarding Form */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <form onSubmit={handleAddVolunteer} className="space-y-3">
              ...
              <button 
                type="submit" 
                className="w-full bg-[#2a9d8f] hover:bg-[#21867a] text-white font-bold py-2 rounded-xl text-xs uppercase tracking-wider transition-all shadow-sm"
              >
      ...
      {/* Comprehensive Operational Edit Modal (RBAC Admin Only) */}
      {editingEvent && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative animate-fadeup">
            <button onClick={() => setEditingEvent(null)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
              ❌
            </button>

            <div>
              <h3 className="text-lg font-bold text-slate-800">Modify Operational Project Drive</h3>
              <p className="text-xs text-slate-400">Edit key relational parameters inside table variables.</p>
            </div>

            <form onSubmit={async (e) => {
              e.preventDefault();
              if (editingEvent) {
                const success = await updateEvent(editingEvent);
                if (success) {
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Drive Title</label>
                <input 
                  type="text" 
                  required
                  value={editingEvent.name}
                  onChange={e => setEditingEvent({ ...editingEvent, name: e.target.value })}
                  className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-[#e63946]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Target City</label>
                  <input 
                    type="text" 
                    required
                    value={editingEvent.location}
                    onChange={e => setEditingEvent({ ...editingEvent, location: e.target.value })}
                    className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-xs outline-none focus:ring-2 focus:ring-[#e63946]"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Target Date</label>
                  <input 
                    type="date" 
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Quota Space</label>
                  <input 
                    type="number" 
                    required
                    min="1"
                    value={editingEvent.requiredVolunteers}
                    onChange={e => setEditingEvent({ ...editingEvent, requiredVolunteers: parseInt(e.target.value) || 1 })}
                    className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-xs outline-none focus:ring-2 focus:ring-[#e63946]"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Project Status State</label>
                  <select 
                    value={editingEvent.status}
                    onChange={e => setEditingEvent({ ...editingEvent, status: e.target.value as any })}
                    className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-xs font-semibold text-slate-600 outline-none hover:bg-slate-100 cursor-pointer"
                  >
                    <option value="Upcoming">Upcoming</option>
                    <option value="Ongoing">Ongoing</option>
                    <option value="Completed">Completed</option>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Overview Description</label>
                <textarea 
                  rows={4}
                  required
                  value={editingEvent.description}
                  onChange={e => setEditingEvent({ ...editingEvent, description: e.target.value })}
                  className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-[#e63946] resize-none leading-relaxed"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  ...
                  className="w-1/2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2.5 rounded-xl text-xs uppercase tracking-wider"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  className="w-1/2 bg-[#e63946] hover:bg-[#c92c38] text-white font-bold py-2.5 rounded-xl text-xs uppercase tracking-wider shadow"
                >
                  Commit Changes
                </button>
              </div>
      ...
      <footer className="bg-slate-900 border-t border-slate-800 text-slate-400 py-10 px-6 text-xs mt-12">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-1.5 text-center md:text-left">
            <h4 className="text-white font-bold text-sm" style={{ fontFamily: "'Pacifico', cursive" }}>HopeKind</h4>
            <p className="text-slate-500">Relational Database Simulator, Client Workspace & Full-Stack Study Companion.</p>
          </div>
          <p className="text-[11px] text-slate-600">© 2026 NGO Fullstack Volunteer Management Ecosystem. All rights reserved.</p>
        </div>
      </footer>
...
// ==========================================
// 4. PRIMARY SYSTEM ENTRANCE POINT (DEFAULT EXPORT)
// ==========================================
